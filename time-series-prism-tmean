...
var temperature = filteredPrism.select(['tmean','t', 'constant'])
  .filterBounds(geometry)
...
var tChart = ui.Chart.image.series(
  temperature.select('tmean'), roi)
  .setChartType('ScatterChart')
  .setOptions({
    title: 'Mean temperature at ROI',
    trendlines: {0: {
      color: 'red'
  }},
  lineWidth: 1,
...
var dependent = ee.String('tmean');

// Compute a linear trend, two bands: residuals and coefficients
var trend = temperature.select(independents.add(dependent))
...
var detrended = temperature.map(function(image){
...
var detrendedChart = ui.Chart.image.series(
  detrended, roi, null, 30)
  .setOptions({
    title: 'Detrended mean temperature time series at ROI',
    lineWidth: 1,
    pointSize: 3,
  });
...
var harmonicTemp = temperature.map(function(image) {
...
var harmonicTrend = harmonicTemp
...
var fittedHarmonic = harmonicTemp.map(function(image) {
...
fittedHarmonic.select(['tmean','fitted']), roi, ee.Reducer.mean(), 30)
    .setSeriesNames(['fitted', 'tmean'])
    .setOptions({
